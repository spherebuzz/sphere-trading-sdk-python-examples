...
class OrderUpdateSubmissionTool:
    ...
    def prompt_and_submit_order_updates(self):
        ...
        while True:
            ...
            try:
                ...
                new_order_request = sphere_sdk_types_pb2.TraderUpdateStripOrderRequestDto(
    ...
    def _submit_order(self, sdk_order_request: sphere_sdk_types_pb2.TraderUpdateStripOrderRequestDto):
        """
        Submit strip order update request.
        """
        logger.info(f"Submitting order with idempotency_key: {sdk_order_request.idempotency_key}")
        
        try:
            orderResponse = self.sdk.update_trader_strip_order(sdk_order_request)
            logger.info(f"Successfully submitted strip order update. Order ID: {orderResponse.id}, Instance ID: {orderResponse.instance_id}")
        except UpdateOrderFailedError as e:
            logger.error(f"Failed to update strip order with Instance ID: {sdk_order_request.instance_id} : {e}")
            raise
        except Exception as e:
